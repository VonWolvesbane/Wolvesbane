...
namespace Server.Mobiles
{
    ...
    public class VendorBackpack : Backpack
    {
        ...
        public override bool CheckItemUse(Mobile from, Item item)
        {
            ...
            if (item is Container || item is Engines.BulkOrders.BulkOrderBook || item is RecipeBook || item is PowerScrollBook)
                return true;
    ...
    public class PlayerVendor : Mobile
    {
        ...
        public int ChargePerDay
        {
            ...
            { 
                if (BaseHouse.NewVendorSystem)
                {
                    return ChargePerRealWorldDay / 2;
                }
                else
                {
                    long total = 0;
        ...
        private class VendorPricePrompt : Prompt
        {
            ...
            private void SetInfo(Mobile from, int price, string description)
            {
                ...
                if (price < 0) // Not for sale
                {
                    ...
                    else if (item is BaseBook || item is Engines.BulkOrders.BulkOrderBook || item is RecipeBook || item is PowerScrollBook
                    )
                    {
                        setPrice = true;
                    }
