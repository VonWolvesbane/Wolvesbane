...
namespace Server.Misc
{
    ...
    public class RegenRates
    {
        ...
        {
            Mobile.DefaultHitsRate = TimeSpan.FromSeconds(9.0);
            Mobile.DefaultStamRate = TimeSpan.FromSeconds(6.0);
        ...
        private static TimeSpan Mobile_ManaRegenRate(Mobile from)
        {
            ...
            if (Core.ML)
            {
                ...
                if (armorPenalty == 0)
                {
                    medBonus = (0.0075 * med) + (0.0125 * from.Int);
                ...
                rate = 3.0 / (0.2 + focusBonus + medBonus + itemBonus);
            }
            else if (Core.AOS)
            {
