...
{
	public class PetEagle : BaseCreature
	{				
		...
		[Constructable]
		public PetEagle() : this( DateTime.MinValue, null, 0 )
		{
		}
		
		[Constructable]
		public PetEagle( DateTime birth, string name, int hue ) : base( AIType.AI_Animal, FightMode.None, 10, 1, 0.2, 0.4 )
		{
			Name = "a pet eagle";
            Title = "the eagle";			
			Body = 5;
		...
		public override void OnStatsQuery( Mobile from )
		{
			if ( from.Map == this.Map && Utility.InUpdateRange( this, from ) && from.CanSee( this ) )
			{
				...
				if ( house != null && house.IsCoOwner( from ) )
                    from.SendMessage("As the house owner, you may rename this Eagle"); // As the house owner, you may rename this Eagle.
					
				from.Send( new Server.Network.MobileStatus( from, this ) );
			}
		}

        public PetEagle(Serial serial)
            : base(serial)
		{
		}				
