...
{ 
   	public class MechanicalPetStatue: Item
   	{ 
		...
		[Constructable]
		public MechanicalPetStatue() : base( 9739 )
		{
			Weight = 0.0;
			Name = "a Mechanical Pet Statue";
			Hue = 1154;
			AllowEvolution = true;
			...
			m_End = DateTime.Now + TimeSpan.FromDays( 1.0 );
            	...
            	public MechanicalPetStatue( Serial serial ) : base ( serial )
            	{             
           	}
		...
		public override void OnDoubleClick( Mobile from )
		{
                        ...
                        { 
                                from.SendMessage( "You must have the Statue in your backpack to release the mechanical pet." );
                        } 
			else if ( this.AllowEvolution == true )
			{
				this.Delete();
				from.SendMessage( "You have now freed the Mechanical Pet from its Statue!!" );

				MechanicalPet dragon = new MechanicalPet ();
			...
			{
				from.SendMessage( "The Mechanical Pet is not yet ready to be released." );
			}
		}
		...
		{ 
			private MechanicalPetStatue de;

			private int cnt = 0;

			public EvolutionTimer( MechanicalPetStatue owner, TimeSpan duration ) : base( duration )
			{ 
				de = owner;
			}
