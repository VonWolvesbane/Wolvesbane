/* Created by Hammerhand*/

using System;
using Server.Items;
using Server.Network;

namespace Server.Items
{
    public class CrashAndBurn : BaseBashing
	{
        public override WeaponAbility PrimaryAbility { get { return WeaponAbility.ConcussionBlow; } }
        public override WeaponAbility SecondaryAbility { get { return WeaponAbility.CrushingBlow; } }

        public override int Hue { get { return 1359; } }
		public override int StrengthReq{ get{ return 60; } }
		public override int MinDamage{ get{ return 18; } }
		public override int MaxDamage{ get{ return 20; } }
        public override float Speed { get { return 2.75f; } }

		public override int InitMinHits{ get{ return 47; } }
		public override int InitMaxHits{ get{ return 115; } }

		[Constructable]
        public CrashAndBurn(): base(0x2D24)
		{
            Name = "Crash And Burn";
            Hue = 1359;
			Weight = 8.0;

            Attributes.AttackChance = Utility.RandomMinMax(8, 25);
            Attributes.BonusDex = Utility.RandomMinMax(3, 12);
            Attributes.WeaponSpeed = Utility.RandomMinMax(4, 17);
            Attributes.SpellChanneling = 1;
            Attributes.Luck = Utility.RandomMinMax(100, 175);
            Attributes.RegenStam = Utility.RandomMinMax(5, 11);
            WeaponAttributes.HitFireArea = Utility.RandomMinMax(12, 23);
            WeaponAttributes.UseBestSkill = 1;
            WeaponAttributes.HitLowerDefend = 15;
            WeaponAttributes.SelfRepair = 5;

            LootType = LootType.Regular;
		}

        public CrashAndBurn(Serial serial): base(serial)
		{
		}

		public override void Serialize( GenericWriter writer )
		{
			base.Serialize( writer );

			writer.Write( (int) 0 ); // version
		}

		public override void Deserialize( GenericReader reader )
		{
			base.Deserialize( reader );

			int version = reader.ReadInt();
		}
	}
}