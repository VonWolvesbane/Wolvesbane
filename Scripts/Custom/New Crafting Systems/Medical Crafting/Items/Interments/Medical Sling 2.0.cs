...
{
	public class MedicalSling : Item, IDyable
	{
		...
		[Constructable]
		public MedicalSling() : this( 1 )
		{
		}

		[Constructable]
		public MedicalSling( int amount ) : base( 0xE21 )
		{
			Name = "a medical sling";
			Stackable = true;
			Amount = amount;
		}

		public MedicalSling( Serial serial ) : base( serial )
		{
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( from.InRange( GetWorldLocation(), Core.AOS ? 2 : 1 ) )
			{
				...
				from.SendLocalizedMessage( 500948 ); // Who will you use the MedicalSlings on?
		...
		{
			private MedicalSling m_MedicalSling;

			public InternalTarget( MedicalSling MedicalSling ) : base( 1, false, TargetFlags.Beneficial )
			{
				m_MedicalSling = MedicalSling;
			}

			protected override void OnTarget( Mobile from, object targeted )
			{
				if ( m_MedicalSling.Deleted )
					return;

				if ( targeted is Mobile )
				{
					if ( from.InRange( m_MedicalSling.GetWorldLocation(), Core.AOS ? 2 : 1 ) )
					{
						if ( MedicalSlingContext.BeginHeal( from, (Mobile) targeted ) != null )
						{
							m_MedicalSling.Consume();
						}
					}
					else
					{
				...
				{
					from.SendLocalizedMessage( 500970 ); // MedicalSlings can not be used on that.
				}
			}
		}
	}

	public class MedicalSlingContext
	{
		...
		public MedicalSlingContext( Mobile healer, Mobile patient, TimeSpan delay )
		{
			m_Healer = healer;
			m_Patient = patient;
		...
		public static MedicalSlingContext GetContext( Mobile healer )
		{
			return (MedicalSlingContext)m_Table[healer];
		}
		...
		public void EndHeal()
		{
			...
			{
				m_Healer.SendLocalizedMessage( 500969 ); // You finish applying the MedicalSlings.
			...
			else
			{
				...
				{
					healerNumber = 500969; // You finish applying the MedicalSlings.
					...
					{
						toHeal = 1;
						healerNumber = 500968; // You apply the MedicalSlings, but they barely help.
					}
		...
		{
			private MedicalSlingContext m_Context;

			public InternalTimer( MedicalSlingContext context, TimeSpan delay ) : base( delay )
			{
				m_Context = context;
		...
		public static MedicalSlingContext BeginHeal( Mobile healer, Mobile patient )
		{
			bool isDeadPet = ( patient is BaseCreature && ((BaseCreature)patient).IsDeadPet );

			if ( patient is Golem )
			{
				healer.SendLocalizedMessage( 500970 ); // MedicalSlings cannot be used on that.
			}
			...
			else if ( healer.CanBeBeneficial( patient, true, true ) )
			{
				...
				MedicalSlingContext context = GetContext( healer );

				if ( context != null )
					context.StopHeal();

				context = new MedicalSlingContext( healer, patient, TimeSpan.FromSeconds( seconds ) );
				...
				healer.SendLocalizedMessage( 500956 ); // You begin applying the MedicalSlings.
				return context;
			}
