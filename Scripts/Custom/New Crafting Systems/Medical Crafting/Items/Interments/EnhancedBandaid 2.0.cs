...
{
	public class EnhancedBandaid : Item, IDyable
	{
		...
		[Constructable]
		public EnhancedBandaid() : this( 1 )
		{
		}

		[Constructable]
		public EnhancedBandaid( int amount ) : base( 0xE21 )
		{
			Name = "a enhanced bandaid";
			Stackable = true;
			Amount = amount;
		}

		public EnhancedBandaid( Serial serial ) : base( serial )
		{
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( from.InRange( GetWorldLocation(), Core.AOS ? 2 : 1 ) )
			{
				...
				from.SendLocalizedMessage( 500948 ); // Who will you use the EnhancedBandaids on?
		...
		{
			private EnhancedBandaid m_EnhancedBandaid;

			public InternalTarget( EnhancedBandaid EnhancedBandaid ) : base( 1, false, TargetFlags.Beneficial )
			{
				m_EnhancedBandaid = EnhancedBandaid;
			}

			protected override void OnTarget( Mobile from, object targeted )
			{
				if ( m_EnhancedBandaid.Deleted )
					return;

				if ( targeted is Mobile )
				{
					if ( from.InRange( m_EnhancedBandaid.GetWorldLocation(), Core.AOS ? 2 : 1 ) )
					{
						if ( EnhancedBandaidContext.BeginHeal( from, (Mobile) targeted ) != null )
						{
							m_EnhancedBandaid.Consume();
						}
					}
					else
					{
				...
				{
					from.SendLocalizedMessage( 500970 ); // EnhancedBandaids can not be used on that.
				}
			}
		}
	}

	public class EnhancedBandaidContext
	{
		...
		public EnhancedBandaidContext( Mobile healer, Mobile patient, TimeSpan delay )
		{
			m_Healer = healer;
			m_Patient = patient;
		...
		public static EnhancedBandaidContext GetContext( Mobile healer )
		{
			return (EnhancedBandaidContext)m_Table[healer];
		}
		...
		public void EndHeal()
		{
			...
			{
				m_Healer.SendLocalizedMessage( 500969 ); // You finish applying the EnhancedBandaids.
			...
			else
			{
				...
				{
					healerNumber = 500969; // You finish applying the EnhancedBandaids.

					double min, max;

					if ( Core.AOS )
					{
						min = (anatomy / 6.0) + (healing / 4.0) + 4.0;
						max = (anatomy / 5.5) + (healing / 1.5) + 4.0;
					}
					else
					{
					...
					{
						toHeal = 1;
						healerNumber = 500968; // You apply the EnhancedBandaids, but they barely help.
					}
		...
		{
			private EnhancedBandaidContext m_Context;

			public InternalTimer( EnhancedBandaidContext context, TimeSpan delay ) : base( delay )
			{
				m_Context = context;
		...
		public static EnhancedBandaidContext BeginHeal( Mobile healer, Mobile patient )
		{
			bool isDeadPet = ( patient is BaseCreature && ((BaseCreature)patient).IsDeadPet );

			if ( patient is Golem )
			{
				healer.SendLocalizedMessage( 500970 ); // EnhancedBandaids cannot be used on that.
			}
			...
			else if ( healer.CanBeBeneficial( patient, true, true ) )
			{
				...
				EnhancedBandaidContext context = GetContext( healer );

				if ( context != null )
					context.StopHeal();

				context = new EnhancedBandaidContext( healer, patient, TimeSpan.FromSeconds( seconds ) );
				...
				healer.SendLocalizedMessage( 500956 ); // You begin applying the EnhancedBandaids.
				return context;
			}
