...
{
	public class BodyCast : Item, IDyable
	{
		...
		[Constructable]
		public BodyCast() : this( 1 )
		{
		}

		[Constructable]
		public BodyCast( int amount ) : base( 0xEC4 )
		{
			Name = "a body cast";
			Stackable = true;
			Amount = amount;
		...
		public BodyCast( Serial serial ) : base( serial )
		{
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( from.InRange( GetWorldLocation(), Core.AOS ? 2 : 1 ) )
			{
				...
				from.SendLocalizedMessage( 500948 ); // Who will you use the BodyCasts on?
		...
		{
			private BodyCast m_BodyCast;

			public InternalTarget( BodyCast BodyCast ) : base( 1, false, TargetFlags.Beneficial )
			{
				m_BodyCast = BodyCast;
			}
			...
			{
				if ( m_BodyCast.Deleted )
					return;

				if ( targeted is Mobile )
				{
					if ( from.InRange( m_BodyCast.GetWorldLocation(), Core.AOS ? 2 : 1 ) )
					{
						if ( BodyCastContext.BeginHeal( from, (Mobile) targeted ) != null )
						{
							m_BodyCast.Consume();
						}
					}
				...
				{
					from.SendLocalizedMessage( 500970 ); // BodyCasts can not be used on that.
				}
			}
		}
	}

	public class BodyCastContext
	{
		...
		public BodyCastContext( Mobile healer, Mobile patient, TimeSpan delay )
		{
			m_Healer = healer;
			m_Patient = patient;
		...
		public static BodyCastContext GetContext( Mobile healer )
		{
			return (BodyCastContext)m_Table[healer];
		}
		...
		{
			private BodyCastContext m_Context;

			public InternalTimer( BodyCastContext context, TimeSpan delay ) : base( delay )
			{
				m_Context = context;
		...
		public static BodyCastContext BeginHeal( Mobile healer, Mobile patient )
		{
			bool isDeadPet = ( patient is BaseCreature && ((BaseCreature)patient).IsDeadPet );

			if ( patient is Golem )
			{
				healer.SendLocalizedMessage( 500970 ); // BodyCasts cannot be used on that.
			}
			...
			else if ( healer.CanBeBeneficial( patient, true, true ) )
			{
				...
				BodyCastContext context = GetContext( healer );

				if ( context != null )
					context.StopHeal();

				context = new BodyCastContext( healer, patient, TimeSpan.FromSeconds( seconds ) );
				...
				healer.SendLocalizedMessage( 500956 ); // You begin applying the BodyCasts.
				return context;
			}
