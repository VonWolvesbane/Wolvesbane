...
{
	public class Bandaid : Item, IDyable
	{
		...
		[Constructable]
		public Bandaid() : this( 1 )
		{
		}

		[Constructable]
		public Bandaid( int amount ) : base( 0xE21 )
		{
			Name = "a bandaid";
			Stackable = true;
			Amount = amount;
		...
		public Bandaid( Serial serial ) : base( serial )
		{
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( from.InRange( GetWorldLocation(), Core.AOS ? 2 : 1 ) )
			{
				...
				from.SendLocalizedMessage( 500948 ); // Who will you use the Bandaids on?
		...
		{
			private Bandaid m_Bandaid;

			public InternalTarget( Bandaid Bandaid ) : base( 1, false, TargetFlags.Beneficial )
			{
				m_Bandaid = Bandaid;
			}
			...
			{
				if ( m_Bandaid.Deleted )
					return;

				if ( targeted is Mobile )
				{
					if ( from.InRange( m_Bandaid.GetWorldLocation(), Core.AOS ? 2 : 1 ) )
					{
						if ( BandaidContext.BeginHeal( from, (Mobile) targeted ) != null )
						{
							m_Bandaid.Consume();
						}
					}
				...
				{
					from.SendLocalizedMessage( 500970 ); // Bandaids can not be used on that.
				}
			}
		}
	}

	public class BandaidContext
	{
		...
		public BandaidContext( Mobile healer, Mobile patient, TimeSpan delay )
		{
			m_Healer = healer;
			m_Patient = patient;
		...
		public static BandaidContext GetContext( Mobile healer )
		{
			return (BandaidContext)m_Table[healer];
		}
		...
		public void EndHeal()
		{
			...
			{
				m_Healer.SendLocalizedMessage( 500969 ); // You finish applying the Bandaids.
			...
			else
			{
				...
				{
					healerNumber = 500969; // You finish applying the Bandaids.

					double min, max;

					if ( Core.AOS )
					{
						min = (anatomy / 7.0) + (healing / 5.0) + 4.0;
						max = (anatomy / 5.9) + (healing / 2.5) + 4.0;
					}
					else
					{
						min = (anatomy / 4.0) + (healing / 4.0) + 3.0;
						max = (anatomy / 3.0) + (healing / 2.0) + 10.0;
					}
					...
					{
						toHeal = 1;
						healerNumber = 500968; // You apply the Bandaids, but they barely help.
					}
		...
		{
			private BandaidContext m_Context;

			public InternalTimer( BandaidContext context, TimeSpan delay ) : base( delay )
			{
				m_Context = context;
		...
		public static BandaidContext BeginHeal( Mobile healer, Mobile patient )
		{
			bool isDeadPet = ( patient is BaseCreature && ((BaseCreature)patient).IsDeadPet );

			if ( patient is Golem )
			{
				healer.SendLocalizedMessage( 500970 ); // Bandaids cannot be used on that.
			}
			...
			else if ( healer.CanBeBeneficial( patient, true, true ) )
			{
				...
				BandaidContext context = GetContext( healer );

				if ( context != null )
					context.StopHeal();

				context = new BandaidContext( healer, patient, TimeSpan.FromSeconds( seconds ) );
				...
				healer.SendLocalizedMessage( 500956 ); // You begin applying the Bandaids.
				return context;
			}
