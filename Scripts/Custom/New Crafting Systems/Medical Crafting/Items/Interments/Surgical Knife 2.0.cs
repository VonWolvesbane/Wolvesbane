...
{
	public class SurgicalKnife : Item, IDyable
	{
		...
		[Constructable]
		public SurgicalKnife() : this( 1 )
		{
		}

		[Constructable]
		public SurgicalKnife( int amount ) : base( 0xEC4 )
		{
			Name = "a surgical knife";
			Stackable = true;
			Amount = amount;
		}

		public SurgicalKnife( Serial serial ) : base( serial )
		{
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( from.InRange( GetWorldLocation(), Core.AOS ? 2 : 1 ) )
			{
				...
				from.SendLocalizedMessage( 500948 ); // Who will you use the SurgicalKnifes on?
		...
		{
			private SurgicalKnife m_SurgicalKnife;

			public InternalTarget( SurgicalKnife SurgicalKnife ) : base( 1, false, TargetFlags.Beneficial )
			{
				m_SurgicalKnife = SurgicalKnife;
			}

			protected override void OnTarget( Mobile from, object targeted )
			{
				if ( m_SurgicalKnife.Deleted )
					return;

				if ( targeted is Mobile )
				{
					if ( from.InRange( m_SurgicalKnife.GetWorldLocation(), Core.AOS ? 2 : 1 ) )
					{
						if ( SurgicalKnifeContext.BeginHeal( from, (Mobile) targeted ) != null )
						{
							m_SurgicalKnife.Consume();
						}
					}
					else
					{
				...
				{
					from.SendLocalizedMessage( 500970 ); // SurgicalKnifes can not be used on that.
				}
			}
		}
	}

	public class SurgicalKnifeContext
	{
		...
		public SurgicalKnifeContext( Mobile healer, Mobile patient, TimeSpan delay )
		{
			m_Healer = healer;
			m_Patient = patient;
		...
		public static SurgicalKnifeContext GetContext( Mobile healer )
		{
			return (SurgicalKnifeContext)m_Table[healer];
		}
		...
		public void EndHeal()
		{
			...
			{
				m_Healer.SendLocalizedMessage( 500969 ); // You finish applying the SurgicalKnifes.
			...
			else
			{
				...
				{
					healerNumber = 500969; // You finish applying the SurgicalKnifes.
					...
					{
						toHeal = 1;
						healerNumber = 500968; // You apply the SurgicalKnifes, but they barely help.
					}
		...
		{
			private SurgicalKnifeContext m_Context;

			public InternalTimer( SurgicalKnifeContext context, TimeSpan delay ) : base( delay )
			{
				m_Context = context;
		...
		public static SurgicalKnifeContext BeginHeal( Mobile healer, Mobile patient )
		{
			bool isDeadPet = ( patient is BaseCreature && ((BaseCreature)patient).IsDeadPet );

			if ( patient is Golem )
			{
				healer.SendLocalizedMessage( 500970 ); // SurgicalKnifes cannot be used on that.
			}
			...
			else if ( healer.CanBeBeneficial( patient, true, true ) )
			{
				...
				SurgicalKnifeContext context = GetContext( healer );

				if ( context != null )
					context.StopHeal();

				context = new SurgicalKnifeContext( healer, patient, TimeSpan.FromSeconds( seconds ) );
				...
				healer.SendLocalizedMessage( 500956 ); // You begin applying the SurgicalKnifes.
				return context;
			}
