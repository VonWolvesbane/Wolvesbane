...
{ 
   	...
   	{ 
		...
		public override void OnDoubleClick( Mobile from )
		{
                        ...
                        { 
                                from.SendMessage( "You must have the Statue in your backpack to release the easter bunny pet." );
                        } 
			else if ( this.AllowEvolution == true )
			{
				this.Delete();
				from.SendMessage( "You have now freed the  Easter Bunny Pet from its Statue!!" );
			...
			{
				from.SendMessage( "The  Easter Bunny Pet is not yet ready to be released." );
			}
		}
