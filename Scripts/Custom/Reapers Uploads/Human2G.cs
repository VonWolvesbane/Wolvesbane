...
namespace Server.Items
{
	public class H2GTarget : Target 
	{
		...
		protected  void OnTarget_OLD(Mobile from, object target)
		{
			...
			if (target is BaseWeapon)
			{
				...
				Item newItem = null;
				if (target is HellSword)
					newItem = Activator.CreateInstance(typeof(DreadSword)) as Item;
				else if (target is BloodyKatana)
					newItem = Activator.CreateInstance(typeof(GlassSword)) as Item;

				if (newItem != null)
				{
					BaseWeapon newWeapon = (BaseWeapon)newItem;
					BaseWeapon oldWeapon = (BaseWeapon)item;
					...
					newWeapon.Hue = oldWeapon.Hue;
					newWeapon.LootType = oldWeapon.LootType;
					newWeapon.Insured = oldWeapon.Insured;

					oldWeapon.OnAfterDuped(newWeapon);

					if (oldWeapon is IDurability && newWeapon is IDurability)
					{
						((IDurability)newWeapon).MaxHitPoints = ((IDurability)oldWeapon).MaxHitPoints;
						((IDurability)newWeapon).HitPoints = ((IDurability)oldWeapon).HitPoints;
					}

					newWeapon.Parent = null;
					newWeapon.Altered = true;

					if (from.Backpack == null)
						newWeapon.MoveToWorld(from.Location, from.Map);
					else
						from.Backpack.DropItem(newWeapon);
			...
			else if (target is HellShield)
			{
				Item item = (Item)Activator.CreateInstance(typeof(SmallPlateShield)) as Item;

				BaseArmor oldarmor = (BaseArmor)target;
				BaseArmor newarmor = (BaseArmor)item;
			...
			else if (target is BaseArmor || target is BaseClothing)
			{
				...
				else if (item.Layer == Layer.Helm) // Make sure its in their pack or they are wearing it
				{
					//Item newitem = typeof(GargishNecklace));
					Item newitem = Activator.CreateInstance(typeof(GargishBandana)) as Item;

					BaseClothing newarmor = (BaseClothing)newitem;
					if (item is BaseArmor)
					{
						...
						newarmor.CopyFromArmor(oldarmor);
						if (oldarmor is IDurability && newarmor is IDurability)
						{
							((IDurability)newarmor).MaxHitPoints = ((IDurability)oldarmor).MaxHitPoints;
							((IDurability)newarmor).HitPoints = ((IDurability)oldarmor).HitPoints;
					...
					{
						BaseClothing oldarmor = (BaseClothing)item;
						...
						newarmor.OnAfterDuped(oldarmor);
						if (oldarmor is IDurability && newarmor is IDurability)
						{
							((IDurability)newarmor).MaxHitPoints = ((IDurability)oldarmor).MaxHitPoints;
							((IDurability)newarmor).HitPoints = ((IDurability)oldarmor).HitPoints;
				...
				else if (item.Layer == Layer.Shoes) // Make sure its in their pack or they are wearing it
				{
					Item newitem = Activator.CreateInstance(typeof(LeatherTalons)) as Item;
				...
				else if (item.Layer == Layer.Waist) // Make sure its in their pack or they are wearing it
				{
					Item newitem = Activator.CreateInstance(typeof(GargishApron)) as Item;
					BaseClothing newarmor = (BaseClothing)newitem;
					if (item is BaseArmor)
					{
						BaseArmor oldarmor = (BaseArmor)item;
