...
namespace VitaNex.InstanceMaps
{
	public sealed class InstanceMap : Map, IEquatable<InstanceMap>
	{
		...
		private static object _RegionsLock = new object();
		private static object _MobilesLock = new object();
		...
		public static void UpdateMaps()
		{
			try
			{
				...
				/*
				var vals = GetMapValues();

				for (var i = 0; i < AllMaps.Count; i++)
				{
					if (vals[i] != AllMaps[i])
					{
						throw new Exception("Map instances did not match at index " + i + ": '" + vals[i] + "' != '" + AllMaps[i] + "'");
					}
				}
				*/
			}
		...
		private void Bounce(Mobile m)
		{
			...
			if (m is BaseCreature)
			{
				...
				if (bc.IsControlled(out master) && bc.Stable(false))
				{
		...
		public void StablePets(PlayerMobile pm)
		{
			...
			pm.AllFollowers.ForEachReverse(
				o =>
				{
		...
		{
			return !ReferenceEquals(other, null) && Serial.Equals(other.Serial);
		}
		...
		public void Deserialize(GenericReader reader)
		{
			...
			switch (version)
			{
				case 0:
				{
					...
					InstanceRegions = reader.ReadBlockList(
						r =>
						{
							var t = r.ReadType();
							...
							if (Regions.ContainsKey(name))
							{
								...
								if (!(reg is InstanceRegion) || !reg.TypeEquals(t, false) || !reg.Area.ContentsEqual(area, true))
								{
									reg.Unregister();
									Regions.Remove(name);
								}
					...
					BounceInfo = reader.ReadBlockDictionary(
						r =>
						{
