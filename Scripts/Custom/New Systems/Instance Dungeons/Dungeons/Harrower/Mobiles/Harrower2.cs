...
{
    public class InstanceHarrower : BaseCreature
    {
            ...
            : base(AIType.AI_NecroMage, FightMode.Closest, 18, 1, 0.2, 0.4)
        {
            ...
            SetSkill(SkillName.MagicResist, 160.2, 190.0);
            SetSkill(SkillName.Magery, 140.0);
            SetSkill(SkillName.EvalInt, 150.0);
            SetSkill(SkillName.Meditation, 420.0);
			
			/*if ( Utility.RandomDouble() < 0.50 )//20% Chance to spawn with a teleporter ticket in its backpack
				{
				switch ( Utility.Random( 1 ) )
					{
					case 0: PackItem(new TeleporterTicket()); break;
					}
				}*/
        ...
        public InstanceHarrower(Serial serial)
            : base(serial)
        {
        }
        ...
        public override int HitsMax
        {
            get
            {
                return m_TrueForm ? 525000 : 400000;
            }
        }
        [CommandProperty(AccessLevel.GameMaster)]
		 ...
		 public override void OnGaveMeleeAttack( Mobile defender )
        {
				...
				if ( 0.1 > Utility.RandomDouble())//10% of the time this weapon will set a defenders hit points to 0 almost killing it
            {
				  ...
				  defender.Say( "Got Scorched by the Harrower!" ); 
                        }
			}
		}
				public override void OnGotMeleeAttack( Mobile attacker )
		{
            ...
            {
				if ( 0.7 > Utility.RandomDouble())//70% of the time this weapon will set a defenders hit points to 0 almost killing it
				{
                  attacker.Hits -=800;
				  attacker.FixedParticles( 0x3709, 10, 30, 5052, EffectLayer.Waist ); //FlameStrike Effect
				  attacker.PlaySound( 0x208 );
				  attacker.Say( "Feels the burn of The Harrower!" ); 
                }
			}
		}
        ...
        public static InstanceHarrower Spawn(Point3D platLoc, Map platMap)
        {
            ...
            InstanceHarrower harrower = new InstanceHarrower();
        ...
        public void Morph()
        {
            ...
            BodyValue = 780;
            Hue = 0x497;
    // Set HitsMax via HitsMaxSeed
    HitsMaxSeed = 1000000;  // Set the max HP to 1 million
    Hits = HitsMax;     
            Stam = StamMax;
            Mana = ManaMax;

 
    // Use SetResistance method to adjust resistances
    ...
    SetResistance(ResistanceType.Energy, 100);
		...
		public override bool OnBeforeDeath()
        {
            if (m_TrueForm)
            {
                ...
                if (!NoKillAwards)
                {
					...
					//Datt99 
					TokenShower.DoForChamp(Location, Map);
		...
		public override void OnDeath(Container c)
        {
            ...
            foreach (Mobile m in rights.Select(x => x.m_Mobile).Distinct())
            {
                if (m is PlayerMobile)
                {
                    switch ( Utility.Random( 2 )) // select which skill to use in the ps
					{ 
                         case 0: m.AddToBackpack(new TeleporterTicket() ); break;
					}
                }			
			}                
		}
