				...
				if ( info.ButtonID == 3 )
				{
					...
					if(tr_Exchange != null)
					{
						...
						if (info.IsSwitched(1)) //buy tokens
						{
							...
							if (daat99.MasterStorageUtils.TakeGoldFromPlayer(m_From as PlayerMobile, iCost))
							{
			...
			public override void OnResponse( NetState state, RelayInfo info ) 
			{
				...
				if ( info.ButtonID == 2 )
				{
					...
					if(tr_BuyTickets != null)
					{
						...
						{
							//daat99.MasterStorageUtils
							if (daat99.MasterStorageUtils.TakeTokensFromPlayer(m_From as PlayerMobile, (i_BuyTickets * i_TicketCost), true))
							{
