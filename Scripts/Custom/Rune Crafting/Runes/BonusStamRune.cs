...
{
	public class BonusStamRune : Item
	{
		[Constructable]
		public BonusStamRune() : base( 0x1F14 )
		{
			Weight = 0.2;  // ?
			Name = "Bonus Stamina Rune";
			Hue = 2101;
		}
		...
		{
			private BonusStamRune m_BonusStamRune;

			public InternalTarget( BonusStamRune runeaug ) : base( 1, false, TargetFlags.None )
			{
				m_BonusStamRune = runeaug;
			}

		 	protected override void OnTarget( Mobile from, object targeted ) 
		 	{ 
				int DestroyChance = Utility.Random( 5 );
				int augment = Utility.Random( 8 ) + 1;
				...
				{ 
						...
						{
							Weapon.Attributes.BonusStam += augment; 
							from.SendMessage( "The Rune enhances your weapon." );
				                  	from.PlaySound( 0x1F5 );
			        	          	m_BonusStamRune.Delete();
			          		}
				...
				{ 
						...
						if ( DestroyChance > 0 ) // Success
						{
							Armor.Attributes.BonusStam += augment; 
				...
				{ 
			       		BaseShield Shield = targeted as BaseShield; 
		       			{
		         			if ( DestroyChance > 0 ) // Success
						{
							Shield.Attributes.BonusStam += augment; 
							from.SendMessage( "The Rune enhances your sheild." );
				                  	from.PlaySound( 0x1F5 );
			        	          	m_BonusStamRune.Delete();
			          		}
				...
				{ 
						...
						if ( DestroyChance > 0 ) // Success
						{
							Clothing.Attributes.BonusStam += augment; 
				...
				{ 
						...
						if ( DestroyChance > 0 ) // Success
						{
							Jewel.Attributes.BonusStam += augment; 
