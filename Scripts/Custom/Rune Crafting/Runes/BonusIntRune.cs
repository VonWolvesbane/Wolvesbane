...
{
	public class BonusIntRune : Item
	{
		[Constructable]
		public BonusIntRune() : base( 0x1F14 )
		{
			Weight = 0.2;  // ?
			Name = "Bonus Intelligence Rune";
			Hue = 2101;
		}
		...
		{
			private BonusIntRune m_BonusIntRune;

			public InternalTarget( BonusIntRune runeaug ) : base( 1, false, TargetFlags.None )
			{
				m_BonusIntRune = runeaug;
			}

		 	protected override void OnTarget( Mobile from, object targeted ) 
		 	{ 
				int DestroyChance = Utility.Random( 5 );
				int augment = Utility.Random( 6 ) + 1;
				...
				{ 
						...
						{						
							Weapon.Attributes.BonusInt += augment;
							from.SendMessage( "The Rune enhances your weapon." );
				                  	from.PlaySound( 0x1F5 );
			        	          	m_BonusIntRune.Delete();
			          		}
				...
				{ 
						...
						if ( DestroyChance > 0 ) // Success
						{
							Armor.Attributes.BonusInt += augment; 
				...
				{ 
						...
						if ( DestroyChance > 0 ) // Success
						{
							Shield.Attributes.BonusInt += augment; 
				...
				{ 
						...
						if ( DestroyChance > 0 ) // Success
						{
							Clothing.Attributes.BonusInt += augment; 
				...
				{ 
						...
						if ( DestroyChance > 0 ) // Success
						{
							Jewel.Attributes.BonusInt += augment; 
