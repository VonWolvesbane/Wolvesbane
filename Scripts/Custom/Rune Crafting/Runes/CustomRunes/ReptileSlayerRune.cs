...
{
	public class ReptileSlayer : Item
	{
		[Constructable]
		public ReptileSlayer() : base( 0x1F14 )
		{
			Weight = 0.2;  // ?
			Name = "Reptile Slayer";
			Hue = 2727;
		...
		{
			private ReptileSlayer m_ReptileSlayer;

			public InternalTarget( ReptileSlayer runeaug ) : base( 1, false, TargetFlags.None )
			{
				m_ReptileSlayer = runeaug;
			}

		 	protected override void OnTarget( Mobile from, object targeted ) 
		 	{ 
				int DestroyChance = Utility.Random( 11 ); // success chance 0-x if 0 fail else success
				...
				{ 
					...
					else if ( Weapon.Slayer == SlayerName.ReptilianDeath || Weapon.Slayer2 == SlayerName.ReptilianDeath )
					{
						...
						{
							Weapon.Slayer = SlayerName.ReptilianDeath; from.SendMessage( "The Rune enhances your weapon." );
				                  	from.PlaySound( 0x1F5 );
			        	          	m_ReptileSlayer.Delete();
			          		}
						else if( Weapon.Slayer2 == SlayerName.None )
						{
							Weapon.Slayer2 = SlayerName.ReptilianDeath; from.SendMessage( "The Rune enhances your weapon." );
				                  	from.PlaySound( 0x1F5 );
			        	          	m_ReptileSlayer.Delete();
			          		}
					      }
		...
		public ReptileSlayer( Serial serial ) : base( serial )
		{
		}
