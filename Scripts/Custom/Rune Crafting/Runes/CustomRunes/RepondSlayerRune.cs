...
{
	public class RepondSlayer : Item
	{
		[Constructable]
		public RepondSlayer() : base( 0x1F14 )
		{
			Weight = 0.2;  // ?
			Name = "Repond Slayer";
			Hue = 2727;
		...
		{
			private RepondSlayer m_RepondSlayer;

			public InternalTarget( RepondSlayer runeaug ) : base( 1, false, TargetFlags.None )
			{
				m_RepondSlayer = runeaug;
			}

		 	protected override void OnTarget( Mobile from, object targeted ) 
		 	{ 
				int DestroyChance = Utility.Random( 11 );
				...
				{ 
					...
					else if ( Weapon.Slayer == SlayerName.Repond || Weapon.Slayer2 == SlayerName.Repond )
					{
						...
						{
							Weapon.Slayer = SlayerName.Silver; from.SendMessage( "The Rune enhances your weapon." );
				                  	from.PlaySound( 0x1F5 );
			        	          	m_RepondSlayer.Delete();
			          		}
						else if( Weapon.Slayer2 == SlayerName.None )
						{
							Weapon.Slayer2 = SlayerName.Silver; from.SendMessage( "The Rune enhances your weapon." );
				                  	from.PlaySound( 0x1F5 );
			        	          	m_RepondSlayer.Delete();
			          		}
					      }
		...
		public RepondSlayer( Serial serial ) : base( serial )
		{
		}
