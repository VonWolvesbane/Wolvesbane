...
namespace Xanthos.Evo
{
	...
	public class Mercenary : BaseEvo, IEvoCreature
	{
		...
		public override void OnSpeech( SpeechEventArgs e )
		{
			...
			switch ( (command == null ? (int)Commands.None : (int)command) )
			{
				...
				case (int)Commands.Stats:
					from.SendGump( new MercLoreGump( ((BaseCreature)this), from, MercLorePage.Stats ) );
					break;
		...
		public override void OnThink()
		{
			...
			if ( Hits < HitsMax - 10 && null == BandageContext1.GetContext( this ))	
			{
				Item item = Backpack.FindItemByType( typeof(EverlastingBandage) );

				if ( null != item && null != BandageContext1.BeginHeal( this , this ))
					item.Consume( 0 );
			}
		}
        ...
        {
			bool isArtifact = Xanthos.Utilities.Misc.IsArtifact( item );

			if ( ( Xanthos.Utilities.Misc.IsPlayerConstructed( item ) || isArtifact ) && !CanUseRestrictedItem( isArtifact ) )
			{
				from.SendMessage( Name + "Must be Max Stage before Equipping Artifacts!" );
				return false;
			}
			
            else if (( ControlMaster != from && this != from ) || IsDeadPet )
				return false;
		...
		public override bool OnDragDrop( Mobile from, Item dropped )
		{
			...
			if ( dropped is Bandage || dropped is Food || dropped is EverlastingBandage)
			{
			...
			else if ( dropped is ShardBreaker )
			{
				from.AddToBackpack( dropped );
				from.SendMessage( "You think you can give this Godly Weapon to a mere Mortal?" );
				return false;
			}
