...
//This method added for [bandself command to call.
		public static void BandSelfCommandCall( Mobile from, Item m_Bandage )
		{
			...
			if ( BandageContext1.BeginHeal( from, from ) != null ) //If this Line throws an error comment it out and uncomment the next line
			...
			else
			{
				...
				if ( chance > Utility.RandomDouble() )
				{
					...
					if ( Core.AOS )
					{
						...
						max = (anatomy / 5.0) + (healing / 4.0) + 7.0;
					}
					else
					{
						min = (anatomy / 8.0) + (healing / 5.0) + 4.0;
