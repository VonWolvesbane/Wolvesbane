...
{ 
   	public class PetMysticAIDeed : Item 
   	{ 
    
      		[Constructable] 
      		public PetMysticAIDeed() : base( 0x14F0 ) 
      		{ 
         		Weight = 1.0;  
         		Movable = true;
         		Name="Pet Mysticism AI Deed";   
      		} 

      		public PetMysticAIDeed( Serial serial ) : base( serial ) 
      		{ 
      		} 
  		...
  		private class AITarget : Target 
      		{ 
         		...
         		private PetMysticAIDeed m_Powder; 

         		public AITarget( PetMysticAIDeed charge ) : base ( 10, false, TargetFlags.None ) 
         		{ 
            			m_Powder=charge; 
         		} 
            			...
            			if( target == from ) 
				{
					...
					else if ( c.Controlled == true && c.ControlMaster == from)
					{
						...
						c.Skills[SkillName.Mysticism].Cap = 200;
						c.Skills[SkillName.Magery].Cap = 200;
						c.RawInt += 100;
						c.Mana += 200;
						from.SendMessage( "Your pet Has Is now learning the ways of a Mystic!" );
						from.PlaySound( 503 );
            					m_Powder.Delete(); 
					}	
