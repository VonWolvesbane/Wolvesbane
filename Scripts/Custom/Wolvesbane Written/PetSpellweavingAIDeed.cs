...
{ 
   	public class PetSpellweavingAIDeed : Item 
   	{ 
    
      		[Constructable] 
      		public PetSpellweavingAIDeed() : base( 0x14F0 ) 
      		{ 
         		Weight = 1.0;  
         		Movable = true;
         		Name="Pet Spellweaving AI Deed";   
      		} 

      		public PetSpellweavingAIDeed( Serial serial ) : base( serial ) 
      		{ 
      		} 
  		...
  		private class AITarget : Target 
      		{ 
         		...
         		private PetSpellweavingAIDeed m_Powder; 

         		public AITarget( PetSpellweavingAIDeed charge ) : base ( 10, false, TargetFlags.None ) 
         		{ 
            			m_Powder=charge; 
         		} 
            			...
            			if( target == from ) 
				{
					...
					else if ( c.Controlled == true && c.ControlMaster == from)
					{
						...
						c.Skills[SkillName.Spellweaving].Cap = 200;
						c.Skills[SkillName.Magery].Cap = 200;
						c.RawInt += 100;
						c.Mana += 200;
						from.SendMessage( "Your pet Has now Started to learn the ways of A Spellweaver" );
						from.PlaySound( 503 );
            					m_Powder.Delete(); 
					}	
