    ...
    public class RunebookGump : Gump
    {
        ...
        private void AddIndex()
        {
            ...
            for (int i = 0; i < 16; ++i)
            {
                ...
                if(i>0 && i<entries.Count)
                    AddButton(237 + ((i / 8) * 168), 63 + ((i % 8) * 15), 2435, 2436, 700 + i, GumpButtonType.Reply, 0);    
                // Move Down button
                if(i<15 && i<entries.Count-1)
                    AddButton(247 + ((i / 8) * 168), 63 + ((i % 8) * 15), 2437, 2438, 800 + i, GumpButtonType.Reply, 0);    
            }
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            else if (buttonID >= 800) //Moving rune down
            {
                ...
                {
                    Book.Entries.Swap<RunebookEntry>(curIndex,newIndex);
                }
                from.CloseGump(typeof(RunebookGump));
