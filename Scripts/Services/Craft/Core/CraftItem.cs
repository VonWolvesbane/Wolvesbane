...
namespace Server.Engines.Craft
{
    ...
    public class CraftItem
    {
        ...
        //Daat99 Resource Table
		//private static readonly Type[][] ItemTypesTable = daat99.ResourceHelper.ItemTypesTable; //GetTypesTable(); //daat99 OWLTR
		//End
		private static readonly Type[][] ItemTypesTable =
		{
			...
			new[] {typeof(EbonyLog), typeof(EbonyBoard)},
			new[] {typeof(EbonyBoard), typeof(EbonyLog)},
			
			new[] {typeof(BambooLog), typeof(BambooBoard)},
			new[] {typeof(BambooBoard), typeof(BambooLog)},
		
			new[] {typeof(PurpleHeartLog), typeof(PurpleHeartBoard)},
			new[] {typeof(PurpleHeartBoard), typeof(PurpleHeartLog)},
			
			new[] {typeof(RedwoodLog), typeof(RedwoodBoard)},
			new[] {typeof(RedwoodBoard), typeof(RedwoodLog)},
			
			new[] {typeof(PetrifiedLog), typeof(PetrifiedBoard)},
            ...
            new[] { typeof( MedusaDarkScales ), typeof( MedusaLightScales ), typeof( RedScales ), typeof( BlueScales ), typeof( BlackScales ), typeof( YellowScales ), typeof( GreenScales ), typeof( WhiteScales ), typeof( CopperScales ), typeof( SilverScales ), typeof( GoldScales ) },
		...
		private static readonly Dictionary<Type, Type> m_ResourceConversionTable = new Dictionary<Type, Type>()
		{
			...
			{ typeof( EbonyLog ), 			typeof( EbonyBoard ) },
			{ typeof( EbonyBoard ), 		typeof( EbonyLog ) },
			
			{ typeof( BambooLog ), 			typeof( BambooBoard ) },
			{ typeof( BambooBoard ), 		typeof( BambooLog ) },
			
			{ typeof( PurpleHeartLog), 		typeof( PurpleHeartBoard ) },
			{ typeof( PurpleHeartBoard ),	typeof( PurpleHeartLog ) },
			
			{ typeof( RedwoodLog ), 		typeof( RedwoodBoard ) },
			{ typeof( RedwoodBoard ), 		typeof( RedwoodLog ) },
			
			{ typeof( PetrifiedLog ), 		typeof( PetrifiedBoard ) },
			...
			bool isFailure)
		{
			...
			for (int i = 0; i < types.Length; ++i)
			{
				...
				// Bugfix: Wood upgrade, normal logs could be made into any coloured board
				if (typeRes != null && baseType != null)
				{
					if (baseType.Equals(typeof(Log)) && !typeRes.Equals(typeof(Board)))
					{
						string woodtype = CraftResources.GetFromType(typeRes).ToString();
						if (!typeRes.FullName.Contains("wood"))
						woodtype = woodtype.Replace("Wood", "");
						baseType = Type.GetType(baseType.FullName.Replace("Log", woodtype + "Log"));
					}
				}
