		...
		public static object CreateItem(IEntity from, string action, out string status_str, Type typerestrict)
		{
			...
			else
			{
				...
				try
				{
					...
					object o = XmlSpawner.CreateObject(type, arglist[0], true);
			...
			if (m != null && !m.Deleted)
			{
				...
				for (int i = 0; i < packlist.Count; ++i)
				{
					...
					if (item != null && !item.Deleted)
					{
						...
						{
							Item itemTarget = BaseXmlSpawner.SearchPackForItem((Container)item, targetName, typeStr);
						...
						if (BaseXmlSpawner.CheckNameMatch(targetName, item.Name))
						{
							if (typeStr == null || BaseXmlSpawner.CheckType(item, typeStr))
							{
								//found it
								return item;
							}
				...
				if (held != null && !held.Deleted && !equippedonly)
				{
					...
					{
						Item itemTarget = BaseXmlSpawner.SearchPackForItem((Container)held, targetName, typeStr);

						if (itemTarget != null) return itemTarget;
					}
					// test the item name against the trigger string
					if (BaseXmlSpawner.CheckNameMatch(targetName, held.Name))
					{
						if (typeStr == null || BaseXmlSpawner.CheckType(held, typeStr))
						{
							//found it
							return held;
						}
			...
			if (m != null && !m.Deleted)
			{
				...
				for (int i = 0; i < packlist.Count; ++i)
				{
					...
					if (item != null && !item.Deleted)
					{
						if (item is Container && !equippedonly)
						{
							itemlist.AddRange(BaseXmlSpawner.SearchPackForItems((Container)item, targetName, typeStr));
						}
						// test the item name against the trigger string
						// if a typestring has been specified then check against that as well
						else if (BaseXmlSpawner.CheckNameMatch(targetName, item.Name))
						{
							if ((typeStr == null || BaseXmlSpawner.CheckType(item, typeStr)))
							{
								//found it
				...
				if (held != null && !held.Deleted && !equippedonly)
				{
					if (held is Container)
					{
						itemlist.AddRange(BaseXmlSpawner.SearchPackForItems((Container)held, targetName, typeStr));
					}
					// test the item name against the trigger string
					else if (BaseXmlSpawner.CheckNameMatch(targetName, held.Name))
					{
						if (typeStr == null || BaseXmlSpawner.CheckType(held, typeStr))
						{
							//found it
