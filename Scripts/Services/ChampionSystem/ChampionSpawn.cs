    ...
    public class ChampionSpawn : Item
    {
        ...
        public void SetWhiteSkullCount(int val)
        {
            ...
            for (int i = m_WhiteSkulls.Count; i < val; ++i)
            {
                ...
                {
                    List<Item> onGround = new List<Item>();
        ...
        public void EndRestart()
        {
            if (RandomizeType)
            {
                switch (Utility.Random(11))
                {
                    ...
                    case 5:
                        Type = ChampionSpawnType.SleepingDragon; break;
                    case 6:
                        Type = ChampionSpawnType.Glade; break;
                    case 7:
                        Type = ChampionSpawnType.Corrupt; break;
                    case 8:
                        Type = ChampionSpawnType.Terror; break;
                    case 9:
        ...
        public void SpawnChampion()
        {
            ...
            if (OWLTROptionsManager.IsEnabled(OWLTROptionsManager.OPTIONS_ENUM.A_LI_N_CLEAN_CHAMP) && OWLTROptionsManager.IsEnabled(OWLTROptionsManager.OPTIONS_ENUM.SAVE_CLEAN_CHAMP_GOLD))
            {
                if (Map != null)
                {
                    ...
                    PublicOverheadMessage(MessageType.Regular, 0, false, "Dropping " + GatheredGold.ToString() + " Gold that was gathered from the Spawn.");
                }
            }
