...
// Wooden, Metal and Metal Golden Chest
	...
	{ 
		...
		{ 
			...
			MaxLockLevel = this.RequiredSkill;
			TrapType = TrapType.MagicTrap;
			TrapPower = 4 * Utility.Random( 5, 75 );

			DropItem( new Gold( 200, 40000 ) );
            ...
            if (0.50 > Utility.RandomDouble()) //75% chance = 3/4
                for (int i = Utility.RandomMinMax(0, 2); i > 0; i--)
                    DropItem(new LuckyHorseshoe());

            if (0.05 > Utility.RandomDouble()) //75% chance = 3/4
                for (int i = Utility.RandomMinMax(0, 1); i > 0; i--)
                    DropItem(new ClothingBlessDeed());

            if (0.05 > Utility.RandomDouble()) //75% chance = 3/4
                for (int i = Utility.RandomMinMax(0, 1); i > 0; i--)
                    DropItem(new ItemBlessDeed());

            if (0.05 > Utility.RandomDouble()) //75% chance = 3/4
                for (int i = Utility.RandomMinMax(0, 1); i > 0; i--)
                    DropItem(new PetMageAIDeed());

            if (0.05 > Utility.RandomDouble()) //75% chance = 3/4
                for (int i = Utility.RandomMinMax(0, 1); i > 0; i--)
                    DropItem(new PetMysticAIDeed());

            if (0.05 > Utility.RandomDouble()) //75% chance = 3/4
                for (int i = Utility.RandomMinMax(0, 1); i > 0; i--)
                    DropItem(new PetNecroAIDeed());

            if (0.05 > Utility.RandomDouble()) //75% chance = 3/4
                for (int i = Utility.RandomMinMax(0, 1); i > 0; i--)
                    DropItem(new PetPaladinAIDeed());

            if (0.05 > Utility.RandomDouble()) //75% chance = 3/4
                for (int i = Utility.RandomMinMax(0, 1); i > 0; i--)
                    DropItem(new PetSpellweavingAIDeed());

            if (0.05 > Utility.RandomDouble()) //75% chance = 3/4
                for (int i = Utility.RandomMinMax(0, 1); i > 0; i--)
                    DropItem(new PetSamuraiAIDeed());

            if (0.05 > Utility.RandomDouble()) //75% chance = 3/4
                for (int i = Utility.RandomMinMax(0, 1); i > 0; i--)
                    DropItem(new PetMaxLevelDeed());

            if (0.05 > Utility.RandomDouble()) //75% chance = 3/4
                for (int i = Utility.RandomMinMax(0, 1); i > 0; i--)
                    DropItem(new SelfRepairRune());
