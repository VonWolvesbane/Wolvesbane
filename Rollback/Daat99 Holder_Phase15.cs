...
namespace Server.Items
{
	public class NewDaat99Holder
	{
		...
		{ 
			writer.Write( (int) 0 ); // legacy holder version
		...
		public void SerializeCompact( GenericWriter writer, Hashtable typeIndex, int catalogCount )
		{
		...
		public NewDaat99Holder( GenericReader reader, Type[] typeCatalog ) // compact deserialize
		{
			...
			switch (version)
			{
				case 1:
				{
					...
					for ( int wordIndex = 0; wordIndex < wordCount; ++wordIndex )
					{
						...
						for ( int bit = 0; bit < 32; ++bit )
						{
							...
							int index = (wordIndex << 5) + bit;
							if ( index >= 0 && index < typeCatalog.Length && typeCatalog[index] != null )
								alItemTypeList.Add( typeCatalog[index] );
						}
					}
				...
				default:
				{
					throw new Exception("Unsupported compact NewDaat99Holder version: " + version);
				}
			}
		}
